...
export class ScreenshotStatisticsAnalyzer {
  ...
  public async topApplicationsByDurationAndProductivity(
    ...
    const qb = this.repository
      ...
      .addSelect('SUM(CASE WHEN metadata.createdAt = activity.createdAt THEN activity.duration ELSE 0 END)', 'totalDuration')
      .addSelect(`SUM(CASE WHEN metadata.createdAt = activity.createdAt AND activity.state = 'active' THEN activity.duration ELSE 0 END)`, 'activeDuration')
